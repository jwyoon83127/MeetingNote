...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initial data state (would normally come from API)
    let meetings = [
    ...
    const views = {
        ...
        meetingDetail: (meetingId) => {
            ...
            if (!meeting) return `<div>회의를 찾을 수 없습니다.</div>`;

            return `
                <div class="space-y-8">
                    <header class="flex items-center gap-4">
                        <button onclick="navigateTo('meetings')" class="p-2 hover:bg-slate-200 rounded-full transition-all text-slate-600">
                            <i data-lucide="arrow-left" class="w-6 h-6"></i>
                        </button>
                        <div>
                            <div class="flex items-center gap-3 mb-1">
                                <h2 class="text-3xl font-bold text-slate-900">${meeting.title}</h2>
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-bold ${meeting.status === '진행중' ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-500'}">
                                    ${meeting.status}
                                </span>
                            </div>
                            <p class="text-slate-500">${meeting.location} • ${meeting.date}</p>
                        </div>
                    </header>
                            ...
                            <div class="space-y-4">
                                ${meetingAgendas.length === 0 ? `
                                    <div class="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                                        <p class="text-slate-400 text-sm">등록된 안건이 없습니다.</p>
                                    </div>
                                ` : meetingAgendas.map((a, idx) => `
                                    <div class="flex items-center justify-between p-5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-brand-200 hover:shadow-sm transition-all">
                                        <div class="flex items-center gap-4">
                                        ...
                                        <span class="text-xs font-bold px-2 py-1 rounded-full ${a.status === '진행중' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-400'}">
                                            ${a.status || '대기중'}
                                        </span>
                                    </div>
                                `).join('')}
    ...
    function navigateTo(viewId, data = null) {
        if (!views[viewId]) {
            // Fallback for missing user template in this snippet
            if (viewId === 'users') { contentArea.innerHTML = '사전 정의된 사용자 관리 템플릿이 로드되지 않았습니다.'; return; }
            return;
        }
        ...
        setTimeout(() => {
            contentArea.innerHTML = views[viewId](data);
    ...
    window.showAgendaForm = (isEdit = false, meetingId = null) => {
        ...
        if (!container) {
            // If we are on meetings view, navigate to agendas view first then show form
            // But for simplicity, let's assume it's there or we provide an inline way.
            // Actually, the agenda form is only in the 'agendas' view template.
            // To make it work from 'meetings' view, we need a better approach.
            navigateTo('agendas');
    ...
    window.hideAgendaForm = () => { document.getElementById('agenda-form-container').classList.add('hidden'); };
    ...
    window.handleAgendaSubmit = (e) => {
        ...
        // If we came from meetings view, go back there
    ...
    window.handleReviewSubmit = (agendaId) => {
        ...
        if (agenda) {
            ...
            agenda.reviews.push({
                user: 'Alice Wong', // Simulated Decision Member
                role: '결정 위원',
                comment: comment,
            ...
            // Refresh view to show new comment
            if (expandedMeetingId) {
                navigateTo('meetings');
            } else {
                navigateTo('agendas');
    ...
    window.hideUserForm = () => { document.getElementById('user-form-container').classList.add('hidden'); };
